...
(function($){
	...
	$.fn.weatherfeed = function(locations, options, fn) {	
		...
		return this.each(function(i, e) {
			...
			if (!$e.hasClass('weatherFeed')) $e.addClass('weatherFeed row');
			...
			var _process = function(e, feed, options) {
				...
				if (feed.description != 'Yahoo! Weather Error') {
					...
					// Add item container
					var html = '<div class="weatherItem col-md-4 col-md-offset-1" ';
					...
					if (options.country) html += '<div class="weatherCountry">Zhejiang,China</div>';
					html += '<div class="weatherTemp">'+ feed.item.condition.temp +'&deg;</div></div>';

					html += '<div class="weatherDetails col-md-6"><div class="weatherDesc">'+ feed.item.condition.text +'</div>';
				
					// Add optional data
					if (options.highlow) html += '<div class="weatherRange">High: <span>'+ wf.high +'&deg;</span> Low: <span>'+ wf.low +'&deg;</span></div>';
					if (options.wind) html += '<div class="weatherWind">Wind: <span>'+ wd +' '+ feed.wind.speed + feed.units.speed +'</span></div>';
					if (options.sunrise) html += '<div class="weatherSunrise">Sunrise: <span>'+ feed.astronomy.sunrise +'</span></div>';
					if (options.sunset) html += '<div class="weatherSunset">Sunset: <span>'+ feed.astronomy.sunset +'</span></div>';
					if (options.humidity) html += '<div class="weatherHumidity">Humidity: <span>'+ feed.atmosphere.humidity +'</span></div>';
					if (options.visibility) html += '<div class="weatherVisibility">Visibility: <span>'+ feed.atmosphere.visibility +'</span></div></div>';
					...
					if (options.forecast) {
						...
						for (var i=0; i<wfi.length; i++) {
							html += '<div class="weatherForecastItem col-md-2" style="background-image: url(http://l.yimg.com/a/i/us/nws/weather/gr/'+ wfi[i].code +'s.png); background-repeat: no-repeat;">';
							...
							html += '<div class="weatherForecastRange">High: <span>'+ wfi[i].high +'</span> Low: <span>'+ wfi[i].low +'</span></div>';
							html += '</div>'
						}
